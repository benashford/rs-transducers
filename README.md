...
`keep` - takes a function of type `Fn(I) -> Option<O>` and returns a `KeepTransducer` that produces all `O`.
...
TODO - other transducers, at a minimum implement all those that Clojure does.  Specifically TODO are: `keep-indexed`, `map-indexed`, `interpose`, `dedupe`, `random-sample`.
